    ...
    <div className="max-w-[1400px] mx-auto px-4 md:px-6 lg:px-12 pt-24 md:pt-32 space-y-24 md:space-y-48 pb-32">
      ...
      <section className="grid lg:grid-cols-[1fr_400px] gap-12 items-center min-h-[60vh]">
        <div className="space-y-6 md:space-y-8">
          ...
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-extrabold tracking-tighter text-zinc-900 dark:text-white leading-[1] lg:leading-[0.9]">
            ...
            <span className="text-zinc-300 dark:text-zinc-700">FOR THE</span> <br className="hidden md:block"/>
          ...
          <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
            <Link to="/contact" className="px-8 py-4 bg-zinc-900 dark:bg-white text-white dark:text-black text-[10px] md:text-xs font-black uppercase tracking-widest hover:bg-indigo-600 dark:hover:bg-indigo-600 hover:text-white dark:hover:text-white transition-all shadow-xl shadow-indigo-500/10 text-center">
              Initiate Project
            </Link>
            <Link to="/projects" className="px-8 py-4 border border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-white text-[10px] md:text-xs font-black uppercase tracking-widest hover:border-zinc-900 dark:hover:border-white transition-all text-center">
              Browse Index
            </Link>
          </div>
        </div>
        ...
        <div className="bg-zinc-50 dark:bg-[#0c0c0e] border border-zinc-200 dark:border-zinc-800 rounded-lg p-4 md:p-6 font-mono text-xs md:text-sm h-[280px] md:h-[320px] shadow-2xl relative flex flex-col">
          ...
          <form onSubmit={handleCommand} className="flex gap-2 text-indigo-600 dark:text-indigo-400 mt-2 border-t border-zinc-200 dark:border-zinc-800 pt-2">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 border-y border-zinc-200 dark:border-zinc-800 py-12 gap-y-12 md:gap-8 reveal">
        {[
          ...
          <div key={stat.label} className="space-y-1 text-center md:text-left">
            ...
            <div className="text-2xl md:text-3xl font-bold text-zinc-900 dark:text-white mono">{stat.value}</div>
          </div>
        ))}
      </section>
      ...
      <section id="skills" className="space-y-12 md:space-y-16 reveal">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-zinc-200 dark:border-zinc-900 pb-8 gap-6">
          <div className="space-y-2">
            <h2 className="text-3xl md:text-4xl font-bold text-zinc-900 dark:text-white tracking-tight uppercase tracking-widest">Technical Arsenal</h2>
          ...
          <div className="flex gap-2 md:gap-4 font-mono text-[9px] md:text-[10px] uppercase tracking-widest overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide">
            {(['all', 'frontend', 'backend', 'devops'] as const).map(cat => (
              <button 
                ...
                className={`px-3 py-1 border transition-all whitespace-nowrap ${activeSkillCategory === cat ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400 bg-indigo-500/5' : 'border-zinc-200 dark:border-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-white'}`}
              >
                {cat}
              </button>
            ))}
          ...
          <div className="lg:col-span-2 space-y-6 overflow-hidden">
            <div className="bg-zinc-50 dark:bg-[#0c0c0e] border border-zinc-200 dark:border-zinc-800 rounded-lg p-4 md:p-6 font-mono text-[10px] md:text-xs overflow-x-auto scrollbar-thin">
              ...
              <div className="space-y-2 min-w-[450px]">
                {filteredSkills.map(skill => (
                  <div key={skill.id} className="grid grid-cols-[50px_1fr_80px_60px_60px] md:grid-cols-[60px_1fr_100px_80px_80px] items-center text-zinc-600 dark:text-zinc-400 hover:bg-white dark:hover:bg-zinc-900/50 py-1 transition-colors group">
                    <span className="text-zinc-400 dark:text-zinc-700">{skill.pid}</span>
                    <span className="font-bold text-zinc-900 dark:text-zinc-200 uppercase truncate pr-2">{skill.name}</span>
                    <div className="flex items-center gap-2 md:gap-3">
                      ...
                      <span className="text-[9px] md:text-[10px] text-zinc-400 dark:text-zinc-600 w-6 md:w-8">{skill.level}%</span>
                    </div>
                    <span className="text-zinc-400 dark:text-zinc-500 uppercase truncate">{skill.category}</span>
            ...
            <div className="bg-zinc-50 dark:bg-[#0c0c0e] border border-zinc-200 dark:border-zinc-800 rounded-lg p-4 md:p-6 space-y-4">
              ...
              <div className="flex flex-wrap gap-1 md:gap-1.5 justify-center sm:justify-start">
                {Array.from({ length: 15 * 10 }).map((_, i) => { // Reduced count for mobile clarity, scale as needed
                  const level = Math.floor(Math.random() * 4);
                  return (
                    <div 
                      key={i} 
                      className={`pixel md:w-3 md:h-3 ${level === 1 ? 'pixel-1' : level === 2 ? 'pixel-2' : level === 3 ? 'pixel-3' : ''} hover:scale-150 transition-transform`}
          ...
          <div className="space-y-6 md:space-y-8">
            ...
            <div className="p-6 border border-indigo-500/20 bg-indigo-500/5 rounded-lg space-y-4">
              ...
              <p className="text-zinc-500 text-[10px] font-mono leading-relaxed">
                Try running <code className="text-indigo-600 dark:text-indigo-300">./skills --frontend</code> in the console above to filter the tech stack.
              </p>
            </div>
          </div>
        </div>
      ...
      <section id="projects" className="space-y-12 md:space-y-16">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end border-b border-zinc-200 dark:border-zinc-900 pb-8 gap-4">
          <h2 className="text-3xl md:text-4xl font-bold text-zinc-900 dark:text-white tracking-tight uppercase tracking-widest">Featured Repositories</h2>
          <Link to="/projects" className="text-indigo-600 dark:text-indigo-400 font-mono text-[10px] md:text-xs hover:text-zinc-900 dark:hover:text-white transition-colors">View All Archive →</Link>
        </div>
            ...
            <div key={project.id} className="group bg-white dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-md p-6 hover:border-indigo-500/50 transition-all duration-300 reveal shadow-sm dark:shadow-none">
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-3 pr-2">
                  ...
                  <h3 className="text-lg md:text-xl font-bold text-zinc-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate">{project.title}</h3>
                </div>
                <span className={`text-[9px] font-bold px-2 py-0.5 rounded border ${project.status === 'production' ? 'border-emerald-500/20 text-emerald-600 dark:text-emerald-500' : 'border-amber-500/20 text-amber-600 dark:text-amber-500'} uppercase tracking-widest shrink-0`}>
                  {project.status}
                </span>
              </div>
      ...
      <footer className="pt-24 md:pt-40 border-t border-zinc-200 dark:border-zinc-900 flex flex-col md:flex-row justify-between items-center gap-8 text-zinc-500 pb-12">
        ...
        <div className="text-[9px] md:text-[10px] font-mono text-center">BUILD_2024.0.1 // SCALEABLE_ARCHITECTURE</div>
      </footer>
    </div>
  );
};
